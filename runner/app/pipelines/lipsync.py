...
class LipsyncPipeline(Pipeline):
    ...
    def generate_real3d_lipsync(self, image_path, audio_path, output_path):
        try: 
            ...
            # Path to the Real3DPortrait Python inference script
            real3dportrait_script = "/models/models--yerfor--Real3DPortrait/inference/real3d_infer.py"
            
            # Virtual environment and env Python binary for real3dportrait
            ...
            print(f"Running real3dportrait inference: {' '.join(run_inference_command)}")

            subprocess.run(run_inference_command, env=env, check=True)
